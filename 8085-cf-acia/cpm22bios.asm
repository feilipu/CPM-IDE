...
; mini-FAT is common/fatfs_85.asm (ROM, no PHASE)
EXTERN  pack_drive
EXTERN  wrdir_cpm
...
rboot:
    call    copy_build      ;RAM copy unroll in ldi_body
...
fcb_clr:
    ...
    ld      (de+),a
    dec     b
    jp      NZ,fcb_clr      ;clear default FCB
...
PUBLIC  copy_build          ;fill ldi_body with 16 * ld a,(hl+)/ld (de+),a + ret
...
copy_build_loop:
    ...
    ld      (hl+),$13       ;inc de
    dec     b
    jp      NZ,copy_build_loop

    ld      (hl),$C9        ;ret
    ret

; IN:  HL = src, DE = dst
; OUT: HL += 128, DE += 128, BC clobbered, AF clobbered
; 16-byte grain: call once for 64, fall through for 64.
ldi_128:
    call    ldi_64
ldi_64:
    ld      bc,ldi_body
...
;*    RAM PHASE. Mini-FAT is common/fatfs_85.asm     *
;*    (linked, no PHASE). wrdir_cpm calls writehst.  *
;*                                                   *
;*****************************************************
...
; IN: A = IDE command, BCDE = LBA. OUT: C if DRQ, NC on ERR/WFT.
; ide_wait_ready spins while busy and returns NC on ERR or WFT.
ide_issue:
    push af
    call ide_wait_ready
    jr NC,ide_issue_nc
...
ldi_body:           defs 65 ;16 * (ld a,(hl+)/ld (de+),a) + ret; filled by copy_build
...
; Always-RAM at $8000+. Kept out of the BIOS PHASE so ACIA rings stay at $FEE0.
