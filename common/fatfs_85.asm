...
fat_sync_window:
    ...
    ld      hl,(fat_winsect+2)
    ld      bc,hl                   ;park winsect high
    ld      hl,(fat_winsect)
    ld      de,(_cpm_fat_vol+20)
    add     hl,de
    ex      de,hl                   ;DE = winsect.lo + fatsz.lo
...
fat_hst_isdir:
    ...
    ld      a,h
    or      a
    ret     NZ                      ;AL >= 256: or a left C clear
    ld      a,l
    cp      DIR_AL
    ret                             ;C if AL < DIR_AL
...
fat_hst_map:
    ...
    call    map_al
    jr      C,fhm_mapped
    ld      a,1                     ;no packed file owns this AL
    ret
fhm_mapped:
...
fhm_fp:
    ...
    ld      hl,fat_work
    call    clst_from_off
    ld      a,3                     ;cluster chain does not reach this block
    ret     NC
    call    clst2sect
    ld      a,3
    ret     NC
...
; User 0 is a real entry. An empty name (00 or space) is padding.
wrdir_slot:
    ld      a,(hl)
    cp      $E5
    jp      Z,wd_era
...
gf_scan:
    ...
    ld      (fat_work+2),hl
    ld      hl,(_cpm_fat_vol+8)     ;sect = fatbase
    ld      (fat_work+4),hl
    ld      hl,(_cpm_fat_vol+10)
    ld      (fat_work+6),hl
    ld      hl,(_cpm_fat_vol+4)     ;remaining = n_fatent
    ld      (fat_work+8),hl
    ld      hl,(_cpm_fat_vol+6)
    ld      (fat_work+10),hl
gf_loop:
    ld      hl,(fat_work+8)
    ld      a,h
